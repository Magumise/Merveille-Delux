   ...
   <form action="" method="post">
      ...
      <h3>filter your search</h3>
         ...
         <div class="flex">
            <div class="box">
               <p>enter location</p>
               <input type="text" name="location" required maxlength="50" placeholder="Kampala" class="input">
            </div>
            <div class="box">
               <p>offer type</p>
...
if(isset($_POST['h_search'])){
   ...
   $select_properties = $conn->prepare("SELECT * FROM `property` WHERE address LIKE '%{$h_location}%' AND type LIKE '%{$h_type}%' AND offer LIKE '%{$h_offer}%' AND price BETWEEN $h_min AND $h_max ORDER BY id DESC");
   ...
   $select_properties = $conn->prepare("SELECT * FROM `property` WHERE address LIKE '%{$location}%' AND type LIKE '%{$type}%' AND offer LIKE '%{$offer}%' AND bhk LIKE '%{$bhk}%' AND status LIKE '%{$status}%' AND furnished LIKE '%{$furnished}%' AND price BETWEEN $min AND $max ORDER BY id DESC");
   $select_properties->execute();

}else{
   $select_properties = $conn->prepare("SELECT * FROM `property` ORDER BY id DESC LIMIT 6");
   $select_properties->execute();
}
      ...
      <form action="" method="POST">
         ...
         <div class="box">
            ...
            <div class="flex">
               ...
               <p><i class="fas fa-maximize"></i><span><?= $fetch_property['carpet']; ?> carpets</span></p>
            </div>
            <div class="flex-btn">
